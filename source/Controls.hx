	...
	public function setHitBox(Hitbox:FlxHitbox, Type:FlxHitboxType)
	{
		switch (Type)
		{
			case SPACE:
				...
				inline forEachBound(Control.KEY5, (action, state) -> addbutton(action, Hitbox.buttonSpace, state));
				inline forEachBound(Control.UP_P, (action, state) -> addbutton(action, Hitbox.buttonUp, state));
				inline forEachBound(Control.DOWN_P, (action, state) -> addbutton(action, Hitbox.buttonDown, state));
				inline forEachBound(Control.LEFT_P, (action, state) -> addbutton(action, Hitbox.buttonLeft, state));
				inline forEachBound(Control.RIGHT_P, (action, state) -> addbutton(action, Hitbox.buttonRight, state));
				inline forEachBound(Control.UP_R, (action, state) -> addbutton(action, Hitbox.buttonUp, state));
				inline forEachBound(Control.DOWN_R, (action, state) -> addbutton(action, Hitbox.buttonDown, state));
				inline forEachBound(Control.LEFT_R, (action, state) -> addbutton(action, Hitbox.buttonLeft, state));
				inline forEachBound(Control.RIGHT_R, (action, state) -> addbutton(action, Hitbox.buttonRight, state));
			case DEFAULT:
				inline forEachBound(Control.UP, (action, state) -> addbutton(action, Hitbox.buttonUp, state));
				inline forEachBound(Control.DOWN, (action, state) -> addbutton(action, Hitbox.buttonDown, state));
				inline forEachBound(Control.LEFT, (action, state) -> addbutton(action, Hitbox.buttonLeft, state));
	...
	public function setVirtualPad(VirtualPad:FlxVirtualPad, DPad:FlxDPadMode, Action:FlxActionMode)
	{
		...
		switch (Action)
		{
			...
			case A_B | A_B_C | A_B_E | A_B_X_Y | A_B_C_X_Y | A_B_C_X_Y_Z | A_B_C_D_V_X_Y_Z:
				...
				inline forEachBound(Control.BACK, (action, state) -> addbutton(action, VirtualPad.buttonB, state));
			case SPACE:
				inline forEachBound(Control.KEY5, (action, state) -> addbutton(action, VirtualPad.buttonSpace, state));
			case NONE: // do nothing
		}
	}
